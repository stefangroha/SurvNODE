...
class ODEFunc(nn.Module):
    ...
    """
    def __init__(self,transition_matrix,num_in,num_latent,layers1,layers2,softplus_beta=1.):
        ...
        self.net1 = nn.Sequential(*((nn.Linear(2*self.num_probs+self.number_of_hazards+num_latent+1,layers1[0]), nn.Tanh()) + tuple(tup for element in tuple(((nn.Linear(layers1[i],layers1[i+1]), nn.Tanh()) for i in range(len(layers1)-1))) for tup in element) + (nn.Linear(layers1[-1],self.number_of_hazards),)))
        self.net2 = nn.Sequential(*((nn.Linear(2*self.num_probs+self.number_of_hazards+2*num_latent+num_in+1,layers2[0]), nn.Tanh()) + tuple(tup for element in tuple(((nn.Linear(layers2[i],layers2[i+1]), nn.Tanh()) for i in range(len(layers2)-1))) for tup in element) + (nn.Linear(layers2[-1],num_latent),)))
        ...
        count = 0
        length = len(list(self.net1.modules()))
    ...
    def forward(self, t, y):
        ...
        out = self.net1(torch.cat((y,torch.tensor([t],device=y.device).repeat((y.shape[0],1))),1))
        # build Q matrix from output
        qvec = torch.nn.functional.softplus(out,beta=self.softplus_beta)
        ...
        out = self.net2(torch.cat((y,self.y0,self.x,torch.tensor([t],device=y.device).repeat((y.shape[0],1))),1))
        return torch.cat((Pprime.reshape(y.shape[0],self.num_probs),Pprime_back.reshape(y.shape[0],self.num_probs),qvec,out),1)
...
#         S = torch.cat([S[i:i+1,from_state[i]-1,from_state[i]-1] for i in range(len(from_state))])
        ...
        # get lambda at tstop
        net_in = torch.cat((torch.cat([out[tstop_indices[i],i:i+1,:] for i in range(len(tstop))]),tstop.reshape(-1,1)),1)
        # net_in = torch.cat((torch.cat([out[tstop_indices[i],i:i+1,:] for i in range(len(tstop))]),self.encoder(x),tstop.reshape(-1,1)),1)
        qvec = torch.nn.functional.softplus(self.odeblock.odefunc.net1(net_in),beta=self.odeblock.odefunc.softplus_beta)
        ...
        out = self.odeblock.odefunc.net2(net_in)
    ...
    def predict_hazard(self,x,tvec):
        ...
        with torch.no_grad():
            ...
            for i in range(tvec.shape[0]):
                net_in = torch.cat((out[i,:,:],tvec[i].repeat(x.shape[0]).reshape(-1, 1)),1)
                temp = self.odeblock.odefunc.net1(net_in)
                qvec = torch.nn.functional.softplus(temp,beta=self.odeblock.odefunc.softplus_beta)
                Q = torch.zeros(self.trans_dim, self.trans_dim,device=x.device).repeat((x.shape[0],1,1))
                Q[self.transition_matrix.repeat((x.shape[0],1,1))==1] = qvec.flatten()
